...
public class GuiListBuilder<T> {
  ...
  private Optional<BiFunction<T, Integer, GuiItemBuilder>> itemCreator =
      Optional.empty();
  private Optional<Runnable> onClose = Optional.empty();
  private SortedMap<Integer, Tuple2<GuiItem, Predicate<GuiListState>>> items =
      new TreeMap<>();
  ...
  public GuiListBuilder<T> rows(int rows) {
    if (rows + 2 > GuiBuilder.MAX_ROWS)
      throw new IllegalArgumentException("Row count cannot exceed " +
                                         (GuiBuilder.MAX_ROWS - 2) + "");
  ...
  public GuiListBuilder<T>
  itemCreation(Function<T, GuiItemBuilder> itemCreator) {
    this.itemCreator = Optional.of((data, index) -> itemCreator.apply(data));
    return this;
  }

  public GuiListBuilder<T>
  itemCreation(BiFunction<T, Integer, GuiItemBuilder> itemCreator) {
  ...
  public GuiListBuilder<T> items(Predicate<GuiListState> condition,
                                 GuiItem item) {
    return items(-1, condition, item);
  }

  public GuiListBuilder<T> items(int index, Predicate<GuiListState> condition,
                                 GuiItem item) {
  ...
  public Gui build() {
    ...
    BiFunction<T, Integer, GuiItemBuilder> generator =
        this.itemCreator.orElse((obj, index) -> {
    ...
    int maxPageCount = (int)Math.floor(Double.valueOf(list.size()) /
                                       Double.valueOf(itemPageCount));
    if (page < 0) {
    ...
    GuiBuilder guiBuilder =
        new GuiBuilder(this.name)
            .rows(this.rowsPerPage + 2)
    ...
    // last row is for navigation
    // max rows count = this.rowsPerPage + 2
    // max slots count = (this.rowsPerPage + 2) * 9
    int maxSlot = (this.rowsPerPage + 2) * 9;

    if (page > 0) {
      // "Previous page" slot = maxSlot - 6;
      int previousSlot = maxSlot - 6;
      guiBuilder.item(
          previousSlot,
          new GuiItemBuilder()
              .name("§7Previous page")
              .icon(Material.ARROW)
              .click(() -> this.pageChange.ifPresent(c -> c.accept(page - 1)))
              .build());
    }
    if (page < maxPageCount) {
      // "Next page" slot = maxSlot - 4;
      int nextSlot = maxSlot - 4;
      guiBuilder.item(
          nextSlot,
          new GuiItemBuilder()
              .icon(Material.ARROW)
  ...
  public void open(Player player) { build().open(player); }
}
