...
public class OGui {
	...
	private void open(Player player) {
		...
		for (var entry : findFields(this.toEditClass).entrySet()) {
			...
			Class<? extends Stringifier> stringifier = Utils.ensureClass(data.stringifier(), Stringifier.class);
			...
			guiBuilder.item(guiItemBuilder.name(fieldName).icon(data.display())
					.descriptionLegacy("§7§oValue: §e" + stringify(value, stringifier)).breakLine()
		...
		for (var entry : findMethods(this.toEditClass).entrySet()) {
			...
			var data = entry.getValue();
			// String methodName = Utils.replaceColor(Utils.orString(data.value(), "&e" +
			// method.getName()));
